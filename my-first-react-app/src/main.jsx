...
import './index.css'
import Institution from './Institutions.jsx'
...
ReactDOM.createRoot(document.getElementById('root')).render(
  <React.StrictMode>
    ...
    {
        institutions &&
        institutions.communityColleges.map((c)=>{return <Institution key= {c.name} name= {c.name} attendance= {c.attendance} study= {c.study} description= {c.description}></Institution>})
    }
    <h2>High School Attended</h2>
    {
        institutions &&
        institutions.highSchool.map((h)=>{return <Institution key= {h.name} name= {h.name} attendance= {h.attendance} study= {h.study} description= {h.description}></Institution>})
    }
  </React.StrictMode>,
)
